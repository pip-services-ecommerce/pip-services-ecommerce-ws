#!/bin/sh

git clone git@github.com:pip-services-ecommerce/pip-facade-ecommerce-node.git

git clone git@github.com:pip-services-ecommerce/pip-services-creditcards-node.git
git clone git@github.com:pip-services-ecommerce/pip-services-paymentmethods-node.git
git clone git@github.com:pip-services-ecommerce/pip-services-reviews-node.git
git clone git@github.com:pip-services-ecommerce/pip-services-reviews-dart.git
git clone git@github.com:pip-services-ecommerce/pip-services-purchaseorders-node.git
git clone git@github.com:pip-services-ecommerce/pip-services-payrollchecks-node.git
git clone git@github.com:pip-services-ecommerce/pip-services-payments-node.git

git clone git@github.com:pip-services-ecommerce/pip-clients-creditcards-node.git
git clone git@github.com:pip-services-ecommerce/pip-clients-paymentmethods-node.git
git clone git@github.com:pip-services-ecommerce/pip-clients-reviews-node.git
git clone git@github.com:pip-services-ecommerce/pip-clients-reviews-dart.git
git clone git@github.com:pip-services-ecommerce/pip-clients-purchaseorders-node.git
git clone git@github.com:pip-services-ecommerce/pip-clients-payrollchecks-node.git
git clone git@github.com:pip-services-ecommerce/pip-clients-payments-node.git
